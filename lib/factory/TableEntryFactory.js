...
var TABLE_ROW_DIV_SNIPPET = '<div class="cpp-field-wrapper cpp-table-row">';
...
function createInputTemplate(properties, canRemove) {
  ...
  forEach(properties, function(prop) {
    template += '<input class="cpp-table-row-columns-' + columns + ' ' +
                               (canRemove ? 'cpp-table-row-removable' : '') + '" ' +
...
function createLabelTemplate(labels) {
  ...
  forEach(labels, function(label) {
    template += '<label class="cpp-table-row-columns-' + columns + '">' + label + '</label>';
  });
  return template;
}
...
module.exports = function(options) {
  ...
  var factory = {
    id: id,
    html: ( canAdd ?
          '<div class="cpp-table-add-row" ' + (canBeShown ? 'data-show="show"' : '') + '>' +
            '<label>' + addLabel + '</label>' +
            '<button class="add" data-action="addElement"><span>+</span></button>' +
          '</div>' : '') +
          '<div class="cpp-table" data-show="showTable">' +
            '<div class="cpp-field-wrapper cpp-table-row">' +
               labelRow +
            '</div>' +
