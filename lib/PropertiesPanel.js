...
PropertiesPanel.prototype.applyValidationResults = function(validationResults, entryNode) {
  ...
  forEach(controlNodes, function(controlNode) {
    ...
    var name = domAttr(controlNode, 'name') || domAttr(controlNode, 'data-name'),
        ...
        messageNode = domQuery('.pp-validation-message', controlNode.parentNode);
    ...
    if (messageNode) {
      domClasses(messageNode).remove('pp-error-message');
      domClasses(messageNode).remove('pp-warning-message');
    ...
    if (message) {
      ...
      if (!messageNode) {
        ...
        domClasses(messageNode).add('pp-validation-message');
      ...
      if (typeof message === 'string' || message.error) {
        ...
        domClasses(messageNode).add('pp-error-message');
      }
      else {
        message = message.warning;

        domClasses(controlNode).add('warning');
        domClasses(messageNode).add('pp-warning-message');
      }
