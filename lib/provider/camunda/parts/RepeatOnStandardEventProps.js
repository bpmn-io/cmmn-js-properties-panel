...
module.exports = function(group, element, cmmnFactory, elementRegistry, itemRegistry, cmmnRules, translate, options) {
  ...
  group.entries = group.entries.concat(TransitionProp(element, translate, {
    ...
    label: translate('Repeat On Standard Event'),
    ...
    validate: function(element, values, node, bo) {
      ...
      if (!isDefaultControl) {
        ...
        if (itemControlRule && itemRepeatOnStandardEvent && isEntryCriteriaAttached(element)) {
          warning = translate('Event is ignored due to entry criteria');
        }
        ...
        if (defaultControlRule && defaultRepeatOnStandardEvent) {
          ...
          if (isEntryCriteriaAttached(element)) {
            ...
            warning = translate('Event is overwritten by item control\'s repetition rule');
          }
